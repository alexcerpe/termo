...
export class ChessRoom extends DurableObject {
  constructor(ctx, env){
    super(ctx,env);this.sockets=new Map();this.state=null;
    ctx.blockConcurrencyWhile(async()=>{this.state=await ctx.storage.get('state')||null;if(this.state)this.ensureState()});
  }
  ensureState(){
    this.state.connected ||= {p1:false,p2:false};this.state.tokens ||= {p1:null,p2:null};this.state.rematch ||= {p1:false,p2:false};this.state.round ||= 1;
  }
  async fetch(req){
    ...
    if(!this.state){if(role!=='p1'){server.close(1008,'Sala ainda não existe');return new Response(null,{status:101,webSocket:client})}this.state=this.newState(minutes);this.state.tokens.p1=token||crypto.randomUUID();await this.save()}
    this.ensureState();
    const known=this.state.tokens[role];
    if(known&&token!==known){server.close(1008,'Vaga ocupada');return new Response(null,{status:101,webSocket:client})}
    if(!known)this.state.tokens[role]=token||crypto.randomUUID();
    ...
    if(this.state.connected.p1&&this.state.connected.p2&&!this.state.ready){this.state.ready=true;this.randomizeColors();await this.save()}
  ...
  async onMessage(socket,event){let m;try{m=JSON.parse(event.data)}catch{return}const role=this.sockets.get(socket);if(!role)return;
    if(m.type==='sync'){this.consumeClock();await this.save();this.broadcast();return}
    if(m.type==='rematch'){if(!this.state.result)return;this.state.rematch[role]=true;if(this.state.rematch.p1&&this.state.rematch.p2){this.resetRound()}await this.save();this.broadcast();return}
    ...
    this.state.fen=chess.fen();this.state.lastMove={from:move.from,to:move.to,san:move.san};this.state.turn=chess.turn();if(!this.state.started)this.state.started=true;
    ...
    this.state.turnStartedAt=this.state.result?null:Date.now();await this.save();this.scheduleAlarm();this.broadcast();
  }
  ...
  publicState(){if(!this.state)return null;const {tokens,...safe}=this.state;return safe}
